...
def calculator():
    ...
    btnclear=tk.Button(roots,text="C",command= clear_field,width=4,font=("Ariel",14))
    btnclear.grid(row=6,column=1,rowspan=2)
    
    btnback=tk.Button(roots,text="<=",command= back,width=4,font=("Ariel",14))
    btnback.grid(row=6,column=2,rowspan=2)
    
    btnequal=tk.Button(roots,text="=",command= evaluate_calculation,width=4,font=("Ariel",14))
    btnequal.grid(row=6,column=3,rowspan=2)
    
    btnexit=tk.Button(roots,text="Go Back",command= exit,width=4,font=("Ariel",14),fg="orange")
    btnexit.grid(row=6,column=4,rowspan=2)
...
def clearcalc():
    f=open("calculatorhistory.csv","w",newline="")
    f.close()
...
def prayer():
    ...
    bg_lbl.place(x=0,y=0,width=700,height=600)
    ...
    frame_one.place(x=80,y=110,width=450,height=140)
    ...
    btn=tk.Button(frame_one,text="Get prayer time",fg="red",font=('times new roman',14,"bold"),command=lambda:prayertime1(txt_box.get(),txt_box.get()),)
    btn.grid(row=0,column=3,sticky="w")
    
    """back=tk.Button(frame_one,text="Go Back",bg="yellow",fg="orange",font=('times new roman',16,"bold"),command=lambda:exit())
    ...
    frame_three=tk.Frame(result,bg="gray",bd=0,highlightthickness=0)
    frame_three.place(relx=0.75,rely=0,relwidth=1,relheight=0.5)
    ...
    #size=int(lab.winfo_height()*0.5)
    #img=ImageTk.PhotoImage(Image.open("icon.webp").resize(size,size))
   
    #lab=tk.Label(result,image=img)
    #ab.pack()
...
def calculation_main():
    ...
    clearhistory_btn=tk.Button(menu, text ="Clear Calculation History",font=('times new roman',16,"bold"),fg="teal",command = clearcalc)
    clearhistory_btn.configure(height=1,width=22)
    clearhistory_btn.grid(row=5,column=1,rowspan=2)
...
exit_btn=tk.Button(menu_frame, text ="Exit the app",font=('times new roman',16,"bold"),fg="violet red",command = exit)
exit_btn.configure(height=1,width=22)
exit_btn.pack(side=tk.TOP,padx=20,pady=20)
